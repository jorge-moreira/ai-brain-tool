...
export async function run(options: { nonInteractive?: boolean } = {}): Promise<void> {
  ensureConfigDir()

  if (options.nonInteractive) {
    await runInstallation({ skipPrompts: true })
    return
  }
...
async function runInstallation(opts: { skipPrompts?: boolean } = {}): Promise<void> {
  ...
  // Step 2: Extras — skip prompts if --yes
  const extras: string[] = []
  if (!opts.skipPrompts) {
