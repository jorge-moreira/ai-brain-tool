...
import { mkdirSync, writeFileSync, rmSync, PathLike } from 'fs'
import { join } from 'path'
import { tmpdir } from 'os'

describe('E2E: Multi-brain scenarios', () => {
  const REPO_PATH = process.env.GITHUB_WORKSPACE || process.cwd()
  let TEMP_DIR: PathLike, BRAIN1_PATH: string, BRAIN2_PATH: string, GLOBAL_CONFIG_DIR
  ...
  beforeAll(() => {
    ...
    brainPaths.forEach(brainPath => {
      ...
      writeFileSync(
        join(brainPath, 'README.md'),
        `# ${brainPath.includes('work') ? 'Work' : 'Personal'} Brain`,
        'utf8'
      )
    })
