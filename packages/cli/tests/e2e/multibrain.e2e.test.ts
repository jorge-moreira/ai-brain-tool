...
describe('E2E: Multi-brain scenarios', () => {
  ...
  it('should update specific brain by id', () => {
    ...
    const output = execSync(
      `bun run ${join(REPO_PATH, 'bin', 'ai-brain.js')} update work 2>&1 || true`,
      {
        encoding: 'utf8',
        env: { ...process.env, __HOME__: TEMP_DIR }
      }
  ...
  it('should update different brain by id', () => {
    ...
    const output = execSync(
      `bun run ${join(REPO_PATH, 'bin', 'ai-brain.js')} update personal 2>&1 || true`,
      {
        encoding: 'utf8',
        env: { ...process.env, __HOME__: TEMP_DIR }
      }
  ...
  it('should show status for specific brain', () => {
    ...
    const output = execSync(
      `bun run ${join(REPO_PATH, 'bin', 'ai-brain.js')} status work`,
      {
        encoding: 'utf8',
        env: { ...process.env, __HOME__: TEMP_DIR }
      }
  ...
  it('should show status for different brain', () => {
    ...
    const output = execSync(
      `bun run ${join(REPO_PATH, 'bin', 'ai-brain.js')} status personal`,
      {
        encoding: 'utf8',
        env: { ...process.env, __HOME__: TEMP_DIR }
      }
  ...
  it('should detect brain from cwd when inside brain folder', () => {
    ...
    const output = execSync(
      `bun run ${join(REPO_PATH, 'bin', 'ai-brain.js')} status 2>&1 || true`,
      {
        encoding: 'utf8',
        cwd: BRAIN1_PATH,
        env: { ...process.env, __HOME__: TEMP_DIR }
  ...
  it('should handle update from cwd without explicit brain id', () => {
    ...
    const output = execSync(
      `bun run ${join(REPO_PATH, 'bin', 'ai-brain.js')} update 2>&1 || true`,
      {
        encoding: 'utf8',
        cwd: BRAIN2_PATH,
        env: { ...process.env, __HOME__: TEMP_DIR }
  ...
  it('should error when invalid brain id specified', () => {
    ...
    const output = execSync(
      `bun run ${join(REPO_PATH, 'bin', 'ai-brain.js')} update nonexistent 2>&1 || true`,
      {
        encoding: 'utf8',
        env: { ...process.env, __HOME__: TEMP_DIR }
      }
