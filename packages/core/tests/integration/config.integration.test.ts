...
import { BrainNotFoundError } from '@ai-brain/core/errors'
...
describe('config integration', () => {
  ...
  describe('isBrainIdAvailable integration', () => {
    it('should return true when brainId is not in config', async () => {
      expect(isBrainIdAvailable('nonexistent-brain')).toBe(true)
    })

    it('should return false when brainId is already registered', async () => {
      const brainPath = join(tmpHome.toString(), 'mybrain')
      mkdirSync(brainPath, { recursive: true })
      addBrain('taken-brain', brainPath)
      expect(isBrainIdAvailable('taken-brain')).toBe(false)
    })
  })

  describe('removeBrain additional integration', () => {
    it('should throw BrainNotFoundError when brain does not exist', async () => {
      // Initialize config first so readConfig() doesn't throw
      ensureConfigDir()
      writeConfig({
        installationComplete: false,
        graphifyyExtras: [],
  ...
  describe('toggleSync integration', () => {
    it('should write gitSync flag to brain config', async () => {
      const brainPath = join(tmpHome.toString(), 'syncbrain')
      mkdirSync(brainPath, { recursive: true })

      toggleSync(brainPath, true)

      const config = readLocalBrainConfig(brainPath)
      expect(config.gitSync).toBe(true)
    })

    it('should disable gitSync when set to false', async () => {
      const brainPath = join(tmpHome.toString(), 'syncbrain2')
      mkdirSync(brainPath, { recursive: true })
      writeBrainConfig({ brainPath, gitSync: true, obsidianDir: null })

      toggleSync(brainPath, false)
  ...
  describe('toggleSyncById integration', () => {
    it('should toggle sync for a registered brain by id', async () => {
      const brainPath = join(tmpHome.toString(), 'byidbrain')
      mkdirSync(brainPath, { recursive: true })
      addBrain('byid-sync-brain', brainPath)

      toggleSyncById('byid-sync-brain', true)
